...
.build/release/herdr-dopa once   # -> on, dopa_pid set (run the nc line again first)
```
...
```sh
.build/release/herdr-dopa daemon &
kill -TERM %1   # after SIGTERM the owned dopa child ends and state.json says off
```

5b. SIGHUP wake test: with `poll_seconds` set high (e.g. `herdr-dopa set
...
```sh
.build/release/herdr-dopa event                                  # no env: like once
HERDR_PLUGIN_EVENT=pane.agent_status_changed \
HERDR_PLUGIN_EVENT_JSON='{"type":"pane.agent_status_changed","agent_status":"working"}' \
  .build/release/herdr-dopa event                                # immediate iteration
HERDR_PLUGIN_EVENT=bogus.event HERDR_PLUGIN_EVENT_JSON='{nope' \
  .build/release/herdr-dopa event                                # still a clean once
```
...
```sh
.build/release/herdr-dopa notify "dopa guard" --body "smoke"
.build/release/herdr-dopa report-metadata   # title + tokens on the owning pane
.build/release/herdr-dopa status --watch 2   # live dashboard, Ctrl+C to exit
```
